...
contract GreaterThanZero {
    // verifies that an amount is greater than zero  验证金额大于零
...
contract ValidAddress {
    // validates an address - currently only checks that it isn't null   验证地址-当前仅检查其是否为null
...
contract OnlyPayloadSize {
	//Mitigate short address attack and compatible padding problem while using “call“  使用“调用”时缓解短地址攻击和兼容的填充问题
...
contract NotThis {
    // verifies that the address is different than this contract address  验证地址与此合同地址不同
...
contract IsContract {
	//assemble the given address bytecode. If bytecode exists then the _addr is a contract.  汇编给定的地址字节码。如果字节码存在，则_addr是合同
...
/**
    Provides support and utilities for contract Creator  //为合约Creato提供支持和实用程序
*/
contract Creator {
    ...
    */
    ...
    // allows execution by the creator only   //只允许创建者执行
    ...
    /**
        ...
        the new creator still needs to accept the transfer   //     新创作者仍然需要接受转让
        can only be called by the contract creator  //     新创作者仍然需要接受转让
        @param _newCreator    new contract creator  //@dev允许转移合同创建权
    */
    ...
    /**
        @dev used by a new creator to accept an Creator transfer   //   新创建者使用的@dev接受创建者转移
    */
...
contract EProToken is SmartToken, Constant, Floor, Sqrt, Formula {
    ...
    function issueToken(address _holder, address _parent, uint256 _value)  internal {
    ...
    function profitingOf(address _holder)  public view returns (uint256){
    ...
    function price()  public view returns (uint256){
        if(m_totalSupply == 0)
            return 0.01 trx;
		return safeDiv(safeMul(safeDiv(safeMul(reserve, MAX_WEIGHT), weight), 1 trx), m_totalSupply);   
    }
    ...
    function calcPurchaseRet(uint256 _value)  public view returns (uint256 amount, uint256 price_, uint256 tax_) {
        ...
        price_ = safeDiv(safeMul(value_85, 1 trx), amount);
    ...
    function calcRedeemRet(uint256 _amount)  public view returns (uint256 value_, uint256 price_, uint256 tax_) {
        ...
        price_ = safeDiv(safeMul(value_, 1 trx), _amount);
